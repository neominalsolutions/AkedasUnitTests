...
namespace Akedas.Accounting.Console.Entites
{
  public class Account
  {
    ...
    public void Deposit(decimal amount)
    {
      ...
      Transactions.Add(new AccountTransaction(Balance, (int)AccountTransactionType.Deposit));
    ...
    {
      decimal dailyCost = Transactions.Where(x => x.TransactionAt.Date == DateTime.Now.Date).Sum(x=> x.Money.Amount);

    

      if(amount > Balance.Amount) // Test Case 1
      {
        throw new Exception("Bakiye yetersiz");
      }
      else if(Closed) // Test Case 2
      {
        throw new Exception("Hesap kapalı para çekilemez");
      }
      else if((dailyCost + amount > 5000))
      {
      ...
      {
        Balance.Amount -= amount;
        Transactions.Add(new AccountTransaction(Balance, (int)AccountTransactionType.WithDraw));
      }
    }
